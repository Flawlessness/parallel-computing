...
int main(int argc, char **argv)
{
    ...
    double *buf0 = (double *) calloc(vecSize[rank], sizeof(double));
    ...
    double *partB = (double *) calloc(vecSize[rank], sizeof(double));
    ...
    MPI_Scatterv(b, vecSize, vecBeginPos, MPI_DOUBLE, partB, vecSize[rank], MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Scatterv(x, vecSize, vecBeginPos, MPI_DOUBLE, partX, vecSize[rank], MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    while (flag)
    {
        ...
        {
            double partX_tmp = partX[i];
            double * partA_tmp = &partA[i * N];
            for (int j = 0; j < N; j++)
            {
                buf2[j] += partA_tmp[j] * partX_tmp;
            }

            buf2[vecBeginPos_tmp + i] = buf2[vecBeginPos_tmp + i] - partB[i];
        }

        MPI_Reduce(buf2, buf1, N, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
        MPI_Bcast(buf1, N, MPI_DOUBLE, 0, MPI_COMM_WORLD);

        for (int i = 0; i < vecSize[rank]; i++)
        {
            partX[i] = partX[i] - T * buf1[vecBeginPos_tmp + i];
        }

        for (int i = 0; i < vecSize[rank]; i++)
        {
            partNorm += buf1[vecBeginPos_tmp + i] * buf1[vecBeginPos_tmp + i];
        }
